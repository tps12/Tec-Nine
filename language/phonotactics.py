...
def constrainonset(syllable, constraints, filler):
    ...
    for n in range(len(syllable.onset), 0, -1):
        ...
        if all([tuple(cluster) in constraints.onsets for cluster in clusters]):
            yield [Syllable(cluster, [filler], []) for cluster in clusters[:-1]] + [Syllable(clusters[-1], syllable.nucleus, syllable.coda)]
        ...
        if tuple(clusters[0]) in constraints.codae and all([tuple(cluster) in constraints.onsets for cluster in clusters[1:]]):
            yield [Syllable([], [filler], clusters[0])] + [Syllable(cluster, [filler], []) for cluster in clusters[1:-1]] + [Syllable(clusters[-1] if len(clusters) > 1 else [], syllable.nucleus, syllable.coda)]

def constraincoda(syllable, constraints, filler):
    if not syllable.coda:
        if () not in constraints.codae:
    ...
    for n in range(len(syllable.coda), 0, -1):
        ...
        if all([tuple(cluster) in constraints.codae for cluster in clusters]):
            yield [Syllable(syllable.onset, syllable.nucleus, clusters[0])] + [Syllable(cluster, [filler], []) for cluster in clusters[1:]]
        ...
        if tuple(clusters[-1]) in constraints.onsets and all([tuple(cluster) in constraints.codae for cluster in clusters[:-1]]):
            yield [Syllable(syllable.onset, syllable.nucleus, clusters[0] if len(clusters) > 1 else [])] + [Syllable(cluster, [filler], []) for cluster in clusters[1:]] + [Syllable(clusters[-1], [filler], [])]

def constrain(syllable, constraints, filler):
    for ss in constrainonset(syllable, constraints, filler):
        for ssc in constraincoda(ss[-1], constraints, filler):
            yield ss[:-1] + ssc
